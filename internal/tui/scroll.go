...
// "> "/"  " (2) + mark (pin/reorder) + head + busy + descendant + att + archived + flag + " ".
const gutterWidth = 10

// gutterHeader is the header text over that gutter — "HBD" (head/busy/descendant) sits
// exactly above those three glyph cells; the mark/att/archived/flag/separator cells are
// blank. Its rune width MUST equal gutterWidth so the column headers line up with the
// data rows (TestGutterHeaderWidth guards this).
