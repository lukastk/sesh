...
## H100 — CONFIGURED DEFAULT AGENT: `[defaults] agent = "pi"`; owner-resolved, explicit override, no hidden built-in (2026-08-29, sesh <this commit> + myrig <this commit>; NO schema/wire change; DAEMON rebuild + restart; NOT DEPLOYED YET)
...
**DEPLOY PLAN.** Commit sesh + myrig, render config on the fleet, build each binary from clean source, restart supervised daemons only with `supervisorctl`; termux uses its explicit-pid/zshenv-guard recipe. Verify config contains agent=pi, installed VCS revision is clean, daemon/mesh health, and an isolated no-`--agent` creation without touching user threads.
