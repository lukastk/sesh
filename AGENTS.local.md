...
DEPLOY: pending the focused commit and fleet rollout. This is daemon code, so installing only a fresh CLI binary is insufficient; each supervised daemon must be restarted through its service manager. Termux remains the explicit-pid/non-supervisor exception documented in earlier entries.
