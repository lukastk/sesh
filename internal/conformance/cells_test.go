...
var skipReasons = map[string]string{
	...
	"thread.whoami":              "the identity GATE: the uuid on stdout only when VERIFIED (pane marker), non-zero otherwise — contradicted env id, uncontradicted-but-unverified env id, or no identity at all",
	...
	"thread.flagged":             "stored needs-attention flag: real unattended turn end auto-flags (per-agent harness hooks); a claude AskUserQuestion stall flags with the question as reason; manual off/disable/enable + flag-on-re-enables; flag_disabled suppresses auto (44)",
	...
	"thread.model":               "spawn/turn with a pinned --model and assert the AGENT actually ran it (pi/claude: model in the transcript; codex: the model string echoed in its loud rejection) + per-turn send-headless --model override",
	...
	"master.remote-work-context": "an ssh master window delegates empty work-server creation to the peer daemon instead of creating it in the ssh session",
