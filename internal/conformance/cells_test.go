...
var skipReasons = map[string]string{
	...
	"thread.new.headless":   "spawn headless thread (pi N/A pending Lukas sign-off)",
	"thread.kill":           "kill a thread",
	"thread.send.headful":   "send into live pane (codex: pending startup-nag/approval handling)",
	"thread.send.headless":  "send as a turn (pi N/A pending Lukas sign-off)",
	"thread.list":           "mesh-replicated cross-machine list",
	"thread.resolve-pane":   "resolve pane via @sesh-thread-id marker",
	"thread.runtime-state":  "activity+attachment axes (codex: pending startup-nag/approval handling)",
	"ticket.create":         "create a ticket",
	"ticket.list-by-thread": "list tickets assigned to a thread",
	"ticket.send-prompt":    "deliver prompt to bound thread",
