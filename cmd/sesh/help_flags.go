...
var flagDocs = map[string][]flagDoc{
	...
	"delegate": {
		...
		{"--keep", "leave the worker un-archived (active) instead of archiving it after the reply"},
		{"--timeout", "give up after this duration (worker is still archived unless --keep; default 10m)"},
	...
	"tui": {
		...
		{"--cwd", "launch-time exact CWD scope: show only threads whose stored working directory equals <dir>; paths inside this user's home compare as owner-portable ~/… across machines; mutually exclusive with --cwd-tree and --cursor"},
		{"--cwd-tree", "launch-time CWD tree scope: show threads at <dir> or in path descendants (path-aware: /work/app2 is not under /work/app); owner-portable within ~; mutually exclusive with --cwd and --cursor"},
		{"--view", "view to show on the first frame: active, on hold, archived, all, or a configured [[tui.views]] name (does not remove the other views)"},
