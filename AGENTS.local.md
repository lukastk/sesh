...
## H77 — long single-line headed sends could fill Codex's composer while their Enter was suppressed (2026-08-01, sesh <this commit>; NO CLI/API/schema change; binary rebuild + supervised daemon restart required; NOT DEPLOYED)
...
DEPLOY: none. The supervised live mymain daemon remains PID 4136119 on exact `dbbc9ce`; no daemon was restarted and no fleet machine runs H77. Once the ITUC workers are quiescent and Lukas accepts the eight unrelated matrix reds, build/install per machine, restart only via each machine's service manager (termux uses its documented exception), and smoke-test a disposable headed Codex thread with repeated >1KiB single-line sends. No cross-host binary refresh is required for compatibility because this changes no API/wire/schema. All six machines are pending deployment.
