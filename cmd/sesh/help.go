...
var helpRegistry = map[string]cmdHelp{
	...
	"tmux info": {
		summary:  "list the work server's tmux sessions as JSONL (one per line); routes cross-machine",
...
does not wrap can be driven straight against the tmux server.`,
	...
	"thread new": {
		...
		examples: []string{"sesh thread new --name default-agent --cwd ~/proj", "sesh thread new --agent claude --name fix-bug --cwd ~/proj", "sesh thread new --agent pi --name notes --cwd . --headless", "sesh thread new --agent pi --name fast --cwd . --headless --model anthropic/claude-haiku-4-5", "exec sesh thread new --agent claude --name here --into-pane $TMUX_PANE --exec", "sesh thread new --virtual --name 'project X'", "sesh thread new --divider --name 'today'"},
	},
	"thread list": {
	...
	"tui": {
		...
		usage:    "sesh tui [--all-machines] [--show-offline] [--cursor] [--filter] [--expand] [--cwd <dir> | --cwd-tree <dir>] [--view <name>] [--sidebar] [--sidebar-filter-style <style>] [--columns <c1,c2,…>] [--editor <cmd>] [--machine <m>]",
		long:     "The grid is driven by a COMMAND PALETTE plus a small fixed keymap. `p` opens the palette: fuzzy-search every command by description or id, ↑/↓ move, enter runs it on the selected thread, esc cancels (a mouse click on a row runs it too). Every action lives there — including the ones with no key at all (go to a thread by uuid, hold-until-a-date, add/remove a tag, set the parent, new virtual group, new divider, pin, fork, delete, show/hide offline machines, quit). `?` lists the whole keymap, one binding per line, scrollable; the bottom line carries only a \"? keys · p commands\" hint. Keys that carry a default binding: ↑/↓ move · ^j/^k scroll · ←/→ fold · ^h/^l pan columns · enter nav · / filter · tab view picker (a popup listing every view, opening on the CURRENT one — tab/↑↓ move, enter or a mouse click applies, esc cancels) · p command palette · f toggle the needs-attention flag (^f toggles auto-flagging) · h hold until tomorrow / release · r rename · u unpin · m move mode (reorder pinned rows) · K tickets · I thread details (a read-only popup of all of the selected thread's fields, scrollable with ↑/↓/^j/^k or the wheel) · i toggle ID column · w toggle the column-width cap · y full uuid · n notify · x stop · a archive INSTANTLY (U undoes the most recent archive — a session-LIFO stack) · R refresh · esc/q quit. ctrl+c also always quits and cannot be rebound, so no config can leave the TUI with no way out. Rebind any command with [[tui.key]] (command = the command id from `?`, key = a key string like \"F\" or \"ctrl+f\"): the first entry for a command REPLACES its default keys, further entries add more, and an empty key unbinds it (leaving it palette-only). An unknown command id, an unusable key name, or two entries fighting over one key is a loud startup error. `goto-uuid` jumps the selection to a thread by uuid: a line prompt takes the full uuid or the short 8-character form, and the cursor lands on that thread — staying in the current view when it already shows it, otherwise switching to the FIRST view in display order that does (so an archived thread lands you in `archived`, a parked one in `on hold`). It LOCATES, it does not enter — Enter still does that. A uuid that matches nothing, matches several threads, or names one the grid is hiding (an offline machine's, a peer's on a self-only grid, one the active filter drops) is refused with a loud line naming the reason. Setting the parent has two commands: `set-parent` opens an interactive picker (type to filter, enter to apply) listing the same machine's threads minus the thread itself, its descendants and dividers, plus a (root) entry that detaches it; `set-parent-uuid` is the original paste-a-uuid prompt. Mouse: click a row to select it, double-click to enter it, click the ▸/▾ fold marker to collapse/expand a subtree; the wheel moves the selection (Shift+wheel pans columns). The default `active` view shows non-archived threads plus archived ones that are still headful or RUNNING, and hides on-hold threads — i.e. (flagged OR not archived OR headful OR running) AND not on hold — so an archived thread stays visible while its agent is working, whether that is a live pane or a HEADLESS turn in flight (◌▶), and drops out once it is quiet; a FLAGGED thread is shown regardless of archived (attention beats parking; unflag to re-hide), but an ON-HOLD thread never shows in active whatever its state (hold beats both flag and running — its ⚑ shows in the `on hold` view); archived rows are marked ⊘ in the state gutter, and PARKED rows carry a hold sigil in the LEADING gutter cell — ⧗ held by its own deadline, ⧖ held by an ancestor's (which cannot be cleared, only released), blank not held; that cell is shared with move mode's ↕, which wins while a row is being moved. Launch-time CWD scopes are orthogonal to both the views and the interactive fuzzy filter: --cwd <dir> admits only threads whose stored working directory exactly matches <dir>; --cwd-tree <dir> also admits path descendants (path-aware, so /work/app2 is not below /work/app). A path inside the invoking user's home is compared owner-portably as ~/… across machines. --view <name> chooses the first view without removing any built-in or configured view; the mt wrappers use --view all. Columns are capped at a max width by default so a long name/cwd can't blow out the layout — `w` toggles the cap off to read clipped text in full; set the default with [tui] max_column_widths and per-column caps with [[tui.column_width]]. Threads on an OFFLINE mesh machine are hidden by default (their owner can't be reached, so entering/mutating them would hang then fail); the OFFLINE footer line shows the count and the toggle-offline command reveals them. Shells view (S): every live tmux session on every reachable machine, classified shell/agent/ghost/stale — the sessions the grid cannot show, because a session sesh did not create has no thread record. It is a list surface like the grid and scrolls like one (↑/↓ or j/k with the viewport following, ^j/^k half-page, / to filter over name+machine+path, wheel to move, click to select, double-click to enter); enter jumps to a session, P promotes it to a tracked shell thread, x kills it (confirmed — the confirmation names the agent threads that would drop to headless), R refreshes, esc closes. Tickets view (K): list a thread's tickets (defaults to ACTIVE; tab filters by status incl. all), drill in, edit name/prompt in $EDITOR, change status/thread, send/delete; n creates a new ticket bound to the thread. --editor sets the field editor (else [tui] editor, else $EDITOR). --sidebar is the persistent-pane mode (issue #8): a narrow name-only layout for a long-lived cockpit pane, where entering a thread does NOT quit the TUI — the nav happens and focus hands to the sibling pane, so the sidebar stays ambient beside the agent — and a SINGLE mouse click enters a thread (a jump list, no double-click; the ▸/▾ marker still folds). Moving the selection FOLLOWS IMMEDIATELY: the cockpit previews the selected thread with focus staying in the sidebar — including across machines (the traveling sidebar rides the master window switch; live headful threads only, a preview never revives; while a preview is in flight further moves coalesce into one catch-up nav). Enter/click commits focus to the thread pane. The cursor also TRACKS the cockpit in the other direction: a thread switch made from the cockpit side moves the selection onto that thread, driven by the bell `sesh tmux nav` rings after every successful nav (read on a cheap timer, with a slower backstop for moves sesh never saw). It moves only when what the master window shows actually CHANGES, so browsing past a row the follow policy skips is never yanked back, and a thread the current view does not contain leaves the cursor alone rather than switching views. esc/q never quit in sidebar mode — the keymap binds them to `dismiss` there instead, clearing the error/note lines (which would otherwise persist forever in a pane that never quits), because a persistent pane must not die to a stray keystroke; a successful nav/follow also clears a stale error; ctrl+c is the deliberate kill and hide/show belongs to the cockpit's toggle binding. A MAXIMIZED sidebar (pane >= 80 cols, e.g. the cockpit's zoom) adaptively renders the full grid column set instead of name-only, and swaps back when restored.",
		examples: []string{"sesh tui --all-machines", "sesh tui --cursor --filter", "sesh tui --cwd . --view all", "sesh tui --cwd-tree ~/mysetup --view all", "sesh tui --editor vim"},
	},
	"info": {
