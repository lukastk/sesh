...
var helpRegistry = map[string]cmdHelp{
	...
	"tui": {
		...
		long:     "Keys (press ? in the TUI for the full keymap popup — one binding per line, scrollable when it overflows the height; the bottom line shows only a \"? keys\" hint): ↑/↓ move · ^j/^k scroll · ←/→ fold · ^h/^l pan columns · enter nav · / filter · tab view picker (a popup listing every view, opening on the CURRENT one — tab/↑↓ move, enter or a mouse click applies, esc cancels) · f toggle the needs-attention flag (^f toggles auto-flagging) · F fork · r rename · t tag · T untag · P parent · v new virtual group (name prompt; created on the selected row's machine; empty cancels) · K tickets · I thread details (a read-only popup of all of the selected thread's fields) · i toggle ID column · w toggle the column-width cap · n notify · x stop · d delete (y/n confirm) · a archive INSTANTLY (U undoes the most recent archive — a session-LIFO stack) · o show/hide offline machines · q quit. Mouse: click a row to select it, double-click to enter it, click the ▸/▾ fold marker to collapse/expand a subtree; the wheel moves the selection (Shift+wheel pans columns). The default `active` view shows non-archived threads plus archived ones that are still headful or RUNNING, and hides on-hold threads — i.e. (flagged OR not archived OR headful OR running) AND not on hold — so an archived thread stays visible while its agent is working, whether that is a live pane or a HEADLESS turn in flight (◌▶), and drops out once it is quiet; a FLAGGED thread is shown regardless of archived (attention beats parking; unflag to re-hide), but an ON-HOLD thread never shows in active whatever its state (hold beats both flag and running — its ⚑ shows in the `on hold` view); archived rows are marked ⊘ in the state gutter. Columns are capped at a max width by default so a long name/cwd can't blow out the layout — `w` toggles the cap off to read clipped text in full; set the default with [tui] max_column_widths and per-column caps with [[tui.column_width]]. Threads on an OFFLINE mesh machine are hidden by default (their owner can't be reached, so entering/mutating them would hang then fail); the OFFLINE footer line shows the count and `o` reveals them. Tickets view (K): list a thread's tickets (defaults to ACTIVE; tab filters by status incl. all), drill in, edit name/prompt in $EDITOR, change status/thread, send/delete; n creates a new ticket bound to the thread. --editor sets the field editor (else [tui] editor, else $EDITOR). --sidebar is the persistent-pane mode (issue #8): a narrow name-only layout for a long-lived cockpit pane, where entering a thread does NOT quit the TUI — the nav happens and focus hands to the sibling pane, so the sidebar stays ambient beside the agent — and a SINGLE mouse click enters a thread (a jump list, no double-click; the ▸/▾ marker still folds). Moving the selection FOLLOWS IMMEDIATELY: the cockpit previews the selected thread with focus staying in the sidebar — including across machines (the traveling sidebar rides the master window switch; live headful threads only, a preview never revives; while a preview is in flight further moves coalesce into one catch-up nav). Enter/click commits focus to the thread pane. esc/q never quit in sidebar mode — they DISMISS the error/note lines instead (which otherwise persist until the next action, i.e. forever in a pane that never quits); a successful nav/follow also clears a stale error; ctrl+c is the deliberate kill and hide/show belongs to the cockpit's toggle binding. A MAXIMIZED sidebar (pane >= 80 cols, e.g. the cockpit's zoom) adaptively renders the full grid column set instead of name-only, and swaps back when restored.",
